<template>
  <div class="landlord-layout">
    ...
    <!-- Blob-фон — один слой на весь лейаут, под всем -->
    ...
    <!-- Стеклянный сайдбар поверх фона -->
    <aside class="sidebar">
      ...
      <div class="sidebar-footer">
        <div class="sidebar-footer-label">Владелец</div>
      </div>
    </aside>

    <!-- Контент — прозрачный, фон просвечивает -->
    <main class="content">
      <router-view />
    </main>
...
const navItems = [
  {
    name: 'dashboard', routeName: 'LandlordDashboard',
    path: '/app/landlord/dashboard', label: 'Дашборд',
  ...
  {
    name: 'properties', routeName: 'LandlordProperties',
    path: '/app/landlord/properties', label: 'Объекты',
    icon: `<svg width="16" height="16" viewBox="0 0 16 16" fill="none">
      <path d="M2 7L8 2l6 5v7a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V7Z" stroke="currentColor" stroke-width="1.3" stroke-linejoin="round"/>
      <path d="M6 15V9h4v6" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/>
    </svg>`,
  },
  {
    name: 'payments', routeName: 'LandlordPayments',
    path: '/app/landlord/payments', label: 'Платежи',
    icon: `<svg width="16" height="16" viewBox="0 0 16 16" fill="none">
  ...
  {
    name: 'requests', routeName: 'LandlordRequests',
    path: '/app/landlord/requests', label: 'Заявки',
    icon: `<svg width="16" height="16" viewBox="0 0 16 16" fill="none">
...
<style scoped>
/* ============ Корень — изоляция и позиционирование ============ */
.landlord-layout {
  position: relative;
  display: flex;
  width: 100%;
...
.sidebar {
  ...
  /* Лёгкий внутренний блик сверху */
  box-shadow:
    1px 0 0 rgba(255, 255, 255, 0.30) inset,
    4px 0 24px rgba(28, 26, 23, 0.04);
  display: flex;
...
.content {
  ...
  overflow-x: hidden;
  min-width: 0;
  /* Намеренно без background — фон просвечивает из .layout-bg */
}
</style>